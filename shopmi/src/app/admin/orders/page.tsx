...
export default function OrdersPage() {
  ...
  const toggleOrderSelection = useCallback((orderId: string) => {
    ...
    const newSelectedData = new Map(selectedOrdersData);
    
    if (newSelected.has(orderId)) {
      newSelected.delete(orderId);
      newSelectedData.delete(orderId);
    } else {
      // Buscar o pedido completo nos dados
      const order = orders.find(o => o.id === orderId);
      if (order && canGenerateLabel(order)) {
        newSelected.add(orderId);
        newSelectedData.set(orderId, order);
      }
    }
    
    setSelectedOrders(newSelected);
    setSelectedOrdersData(newSelectedData);
  }, [selectedOrders, selectedOrdersData, orders, canGenerateLabel]);
  ...
  const openLabelModal = useCallback(() => {
    // Usar os dados já armazenados no Map
    const selectedOrdersList = Array.from(selectedOrdersData.values());
  ...
  const onSelectionChanged = useCallback(() => {
    ...
    const newSelectedData = new Map<string, Order>();
    
    selectedNodes.forEach((node: any) => {
      if (canGenerateLabel(node.data)) {
        newSelected.add(node.data.id);
        newSelectedData.set(node.data.id, node.data);
      }
    });
  ...
  const selectedEligibleCount = useMemo(() => 
    Array.from(selectedOrdersData.values()).length,
    [selectedOrdersData]
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <LabelGenerationModal
          ...
          selectedOrders={Array.from(selectedOrdersData.values())}
